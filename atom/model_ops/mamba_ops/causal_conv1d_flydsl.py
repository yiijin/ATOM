...
def build_causal_conv1d_flydsl_v2_module(
    ...
    """flydslv2: idiomatic FlyDSL causal conv1d, bit-exact to the hand-written
    HIP ``conv1d_v11_t`` kernel.

    FlyDSL API surface is kept idiomatic for the installed toolchain
    (cf. ``kernels/pa_decode_fp8.py``):

      * **Inputs are ``fx.Tensor``**; the buffer resources are built with
        ``buffer_ops.create_buffer_resource`` and addressed with explicit
        element strides, exactly like ``pa_decode``.
      * **Runtime control flow uses plain Python ``if``/``else``** which the
        FlyDSL AST rewriter lowers to ``scf.if`` (value-less branches, so no
        results are yielded) -- no explicit ``scf.IfOp`` +
        ``with ir.InsertionPoint(...)`` + ``scf.YieldOp([])`` boilerplate.
      * **Hot LDS staging uses ``SmemAllocator``/``SmemPtr``** (standard
        ``memref`` dialect over a ``memref.global_``), which lowers straight to
        ds_read/ds_write. The ``rmsnorm``-style ``fx.SharedAllocator`` +
        ``fx.struct`` + ``.view`` + ``fx.memref_load``/``fx.memref_store``
        (``fly`` dialect) path was measured ~4% slower here: its
        int_tuple/layout coordinate arithmetic does not fully fold for this
        staging pattern.

    Requires the local FlyDSL (>= 0.2.0, ``/workspace/FlyDSL``): the ``if`` ->
    ``scf.if`` AST rewrite is a 0.2.0 feature. Compute notes:
